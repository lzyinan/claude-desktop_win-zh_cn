...
# so existing tests that mock patch_chunks.shutil.copy2 keep their anchor points.
...
def backup_file(path: Path, assets_dir: Path) -> None:
    """Back up a chunk file under BACKUP_ROOT (relative to assets_dir).

    Thin wrapper around best_effort_io.backup_file so this module keeps its
    historical (path, assets_dir) signature and module-level BACKUP_ROOT,
    which existing tests rely on.
    """
    best_effort_io.backup_file(path, assets_dir, backup_root=BACKUP_ROOT)
